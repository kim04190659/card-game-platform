  ...
  try {
    ...
    const mockEvaluations = {
      ...
      'otasuke-san': {
        ...
        reasons: {
          'わかりやすさ': '大きな文字、番号付きリスト、絵文字を活用し、81歳の方でも理解しやすい。専門用語を使わず、シンプルで明確な表現が素晴らしい。',
          '優しさ': '「大丈夫ですよ」という安心感と、励ましの言葉が素晴らしい。上から目線ではなく、対等な友人のように接している点が評価できる。',
          '実用性': '受付で見せるメモがあり、すぐに使える実用的な内容。耳が聞こえづらい方への配慮が行き届いており、筆談の依頼文も完璧。'
        },
        improvements: [
          '病院までの行き方も追加すると、さらに親切になります',
          '写真付きで説明すると、より分かりやすくなります',
          '受付で見せるメモを、大きな文字で別ページに表示すると、さらに便利です'
        ],
        overallScore: 95,
        feedback: '高齢者に寄り添った、素晴らしい内容です。特に「受付で見せるメモ」が実用的で、耳が聞こえづらい方への配慮が行き届いています。絵文字の使い方も効果的で、温かみのある文章になっています。「大丈夫ですよ」という励ましの言葉が、安心感を与えてくれます。',
        mockMode: true
      }
    };
